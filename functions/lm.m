...
%    opts = [  3,10*Npar, 1e-3, 1e-3, 1e-3, 1e-2, 1e-2,    11,    9,        1 ];
   opts = [  3,20*Npar, 1e-3, 1e-3, 1e-3, 1e-2, 1e-2,    11,    9,        1 ];
...
%       h = ( JtWJ + lambda*diag(diag(JtWJ)) ) \ JtWdy;
        h = ( JtWJ + lambda*diag(diag(JtWJ)) + (alpha^2)*(L'*L) ) \ (JtWdy - (alpha^2)*(L'*(L*p_try))); % with regularization
%         h = ( JtWJ + lambda*diag(diag(JtWJ)) ) \ (JtWdy ); % with regularization
    otherwise					% Quadratic and Nielsen
...
%    delta_y = y_dat - feval(func,t,p_try,c);       % residual error using p_try
   delta_y = y_dat - feval(func,p_try);       % residual error using p_try
...
%    figure(2)
%    hold on
%    plot( rStand(J(1:4:end,5)),'-.r')
%    plot(rStand(J(1:4:end,4)),'-ob')
%    plot(rStand(J(1:4:end,3)),'->k')
%    plot(rStand(J(1:4:end,2)),'-+g')
%    plot(rStand(J(1:4:end,1)),'-*c')
%    hold on
%    plot(J(:,1))
